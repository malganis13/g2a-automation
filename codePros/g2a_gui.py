...
"""
    ...
    def load_price_stats(self, period):
        """Загрузка статистики цен (ИСПРАВЛЕНО - работает с localhost)"""
        def load_stats():
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            ...
            try:
                ...
                # ✅ ИСПРАВЛЕНИЕ: Перезагружаем конфиг перед запросом
                import g2a_config

                from g2a_config import API_BASE_URL, ADMIN_API_KEY

                # ✅ ИСПРАВЛЕНИЕ: Проверяем что API_BASE_URL правильный
                if "ngrok.app" in API_BASE_URL:
                    raise Exception(
                        f"❌ ОШИБКА КОНФИГУРАЦИИ!"
                        f"API_BASE_URL содержит ngrok.app:"
                        f"'{API_BASE_URL}'"
                        f"Это значит что ngrok токен истек!"
                        f"ИСПРАВЛЕНИЕ:"
                        f"1. Открой g2a_config.py"
                        f"2. Найди API_BASE_URL = \"https://domen.ngrok.app\""
                        f"3. Замени на: API_BASE_URL = \"http://localhost:8000\""
                        f"4. Перезапусти софт"
                    )
                ...
                output = f"""
╔════════════════════════════════════════════════════════╗
║ 📊 СТАТИСТИКА ИЗМЕНЕНИЙ ЦЕН - {data.get('period', 'N/A')}
╚════════════════════════════════════════════════════════╝

📈 ОБЩАЯ СВОДКА:
───────────────────────────────────────────────────────────
Всего изменений цен: {data.get('summary', {}).get('total_changes', 0)}
📉 Понижений цен: {data.get('summary', {}).get('price_decreases', 0)}
📈 Повышений цен: {data.get('summary', {}).get('price_increases', 0)}
💰 Среднее изменение: €{data.get('summary', {}).get('avg_price_change', 0):.2f}
💸 Общее изменение: €{data.get('summary', {}).get('total_price_change', 0):.2f}
🕐 Изменений сегодня: {data.get('summary', {}).get('today_changes', 0)}
"""
...
{idx}. {game['game_name']}
   ...
   Мин. старая: €{game['min_old_price']:.2f}
   Макс. новая: €{game['max_new_price']:.2f}
   Среднее: €{game['avg_change']:.2f}
"""
...
"""
                    for change in data['recent_changes'][:50]:
                        ...
                        output += f"""
{change['created_at']}
{direction} {change['game_name']}
   Старая: €{change['old_price']:.2f} → Новая: €{change['new_price']:.2f}
   Рынок: €{change.get('market_price', 'N/A')}
   Причина: {change.get('change_reason', 'N/A')}
───────────────────────────────────────────────────────────
"""

                    if len(data['recent_changes']) > 50:
                        output += f"... и еще {len(data['recent_changes']) - 50} изменений"
...
1️⃣ API СЕРВЕР НЕ ЗАПУЩЕН
   └─ Убедись что g2a_fastapi_server.py запущен

2️⃣ НЕВЕРНЫЙ API_BASE_URL
   └─ Открой g2a_config.py и проверь:
   └─ Должно быть: http://localhost:8000
   └─ НЕ должно быть: https://domen.ngrok.app

3️⃣ НЕВЕРНЫЙ API KEY
   └─ Проверь ADMIN_API_KEY в g2a_config.py

4️⃣ НЕТ ДАННЫХ ЗА ПЕРИОД
   └─ Должны быть хотя бы какие-то изменения цен
...
□ Запустить FastAPI сервер:
  python g2a_fastapi_server.py

□ Проверить g2a_config.py:
  API_BASE_URL = "http://localhost:8000"
  ADMIN_API_KEY = "ваш-ключ"

□ Запустить автоизменение цен (чтобы была статистика):
  Вкладка "Автоизменение" → нажать "Запустить"
